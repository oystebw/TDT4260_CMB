...
__attribute__((hot)) void blurIterationVertical4(v4Accurate* restrict in, v4Accurate* restrict out, const int size, const int width, const int height) {
	register const v4Accurate multiplier = (v4Accurate){(2 * size + 1), (2 * size + 1), (2 * size + 1), 1.0f};
	#pragma omp parallel for simd schedule(dynamic, 2) num_threads(8)
	for(int x = 0; x < width; ++x) {
		register const int xHeight = x * height;
...
__attribute__((hot)) void blurIterationVerticalAndDiff(PPMPixel* restrict result, const v4Accurate* restrict small, const v4Accurate* restrict in, v4Accurate* restrict out, const int size, const int width, const int height) {
	...
	for(int x = 0; x < width; ++x) {
		...
		register v4Accurate* restrict res = &out[xHeight + size + 1];
		register const v4Accurate* restrict resSmall = &small[xHeight + size + 1];
		register const v4Accurate* restrict minus = &in[xHeight];
		register const v4Accurate* restrict plus = &in[xHeight + size * 2 + 1];
		#pragma GCC unroll 16
...
int main(int argc, char** argv) {
	...
	blurIterationVertical4(scratch, two, 3, width, height);
	blurIterationVerticalAndDiff(result->data, one, scratch, two, 3, width, height);
	...
	blurIterationVertical4(scratch, one, 5, width, height);
	...
	blurIterationVertical4(scratch, two, 8, width, height);
