...
public:
    OpenClBlur() {
        ...
        kernelDiff = Kernel(program, "kernelDiff");
    }

    void imageDiff(PPMPixel* result, const v4Accurate* large, const v4Accurate* small, const int width, const int height){
        const int size = width * height;
        Buffer largebuff(context, CL_MEM_READ_WRITE|CL_MEM_READ_WRITE, size * sizeof(v4Accurate));
        Buffer smallbuff(context, CL_MEM_READ_WRITE|CL_MEM_READ_WRITE, size * sizeof(v4Accurate));
        Buffer resultbuff(context, CL_MEM_READ_WRITE|CL_MEM_READ_WRITE, size * sizeof(PPMPixel));

        events.emplace_back(make_pair("copy buffer to image", Event()));

        queue.enqueueWriteBuffer(largebuff, false, 0, size * sizeof(v4Accurate), large, nullptr, &events.back().second);
        queue.enqueueWriteBuffer(smallbuff, false, 0, size * sizeof(v4Accurate), small, nullptr, &events.back().second);

        diff(resultbuff, largebuff, smallbuff, width, height);

        events.emplace_back(make_pair("map buffer in memory", Event()));

        result = (PPMPixel*)queue.enqueueMapBuffer(resultbuff, CL_FALSE, CL_MAP_READ, 0, size * sizeof(PPMPixel), nullptr, &events.back().second);
    }
    void finish(){
...
private:
    ...
    void diff(Buffer& resultbuff, Buffer&largebuff, Buffer& smallbuff, const int width, const int height){
        events.emplace_back(make_pair("kernelDiff", Event()));
        // set call arguments
        kernelDiff.setArg(0, resultbuff);
        kernelDiff.setArg(1, largebuff);
        kernelDiff.setArg(2, smallbuff);
        kernelDiff.setArg(3, height);
        // call 2D kernel
        queue.enqueueNDRangeKernel(
                kernelDiff, // kernel to queue
                NullRange, // use no offset
                NDRange(width), // 1D kernel
...
int main(int argc, char** argv) {
    ...
    OpenClBlur blur;
    blur.imageDiff(result_data, two, one, width, height);
    blur.finish();
