...
PPMImage* readStreamPPM(FILE*  fp) {
	...
	image = (PPMImage* )aligned_alloc(16, sizeof(PPMImage));
	if (!image) {
	...
	image->data = (PPMPixel* )aligned_alloc(16, image->x * image->y * sizeof(PPMPixel));
...
PPMImage* readPPM(const char*  filename)
{
    ...
    //alloc memory form image
    img = (PPMImage* )aligned_alloc(16, sizeof(PPMImage));
    if (!img) {
         fprintf(stderr, "Unable to allocate memory\n");
         exit(1);
    }
    ...
    while (fgetc(fp) != '\n') ;
    //memory allocation for pixel data
    img->data = (PPMPixel* )aligned_alloc(16, img->x * img->y * sizeof(PPMPixel));
